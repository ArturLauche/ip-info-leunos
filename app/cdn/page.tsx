      ...
      <div className="z-10 flex w-full max-w-5xl flex-col items-center gap-8 px-4 py-10 md:py-16">
        <header className="flex w-full max-w-2xl flex-col items-center gap-6 text-center">
          ...
          <div className="flex flex-col items-center gap-2">
            <h1 className="text-2xl font-semibold tracking-tight text-foreground md:text-3xl">{t.cdnTitle}</h1>
            <p className="max-w-xl text-sm text-muted-foreground md:text-base">{t.cdnSubtitle}</p>
          </div>
        </header>

        <section className="surface-panel w-full">
          <CdnChecker locale={locale} />
        </section>
      </div>
    </main>
  );
