...
import { resolveLanguage, uiText } from "@/lib/i18n";
...
export default async function Home({ searchParams }: HomeProps) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center px-4 py-16 md:py-24">
      <LanguageSwitcher current={lang} path="/" />
      ...
      <header className="mb-12 flex flex-col items-center gap-4">
        ...
        <div className="flex flex-col items-center gap-1">
          <h2 className="text-lg font-semibold text-foreground">{text.appTitle}</h2>
          <p className="text-sm text-muted-foreground">{text.appSubtitle}</p>
        </div>
      </header>
