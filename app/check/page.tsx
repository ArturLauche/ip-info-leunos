...
export default async function CheckPage({ searchParams }: CheckPageProps) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center px-4 py-16 md:py-24">
      <LanguageSwitcher current={lang} path="/check" />
      ...
      <header className="mb-12 flex flex-col items-center gap-4">
        ...
        <div className="flex flex-col items-center gap-1">
          <h2 className="text-lg font-semibold text-foreground">{text.checkTitle}</h2>
          <p className="text-sm text-muted-foreground">{text.checkSubtitle}</p>
        </div>
      </header>
