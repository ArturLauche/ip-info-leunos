...
function headlineLabel(summary: ReputationSummary, t: ToolT) {
  if (summary.headline === "no_malicious_activity")
    return t.reputationHeadlineClean;
...
function stateVariant(
  status: SourceStatus,
): "destructive" | "warning" | "success" | "info" | "secondary" {
  ...
  return (
    <div className="flex flex-col gap-2 border-b px-5 py-4 last:border-b-0">
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-2 rounded-md bg-muted/30 px-3 py-2.5 sm:grid-cols-2">
        <MetaRow
          label={t.reputationFieldSource}
          value={sourceName(item.sourceId)}
        />
        {confidence !== null && (
          <MetaRow label={t.reputationFieldConfidence} value={confidence} />
        )}
        {firstSeen !== null && (
          <MetaRow label={t.reputationFieldFirstSeen} value={firstSeen} />
        )}
        {lastSeen !== null && (
          <MetaRow label={t.reputationFieldLastSeen} value={lastSeen} />
        )}
        ...
        {item.malwareFamily && (
          <MetaRow
            label={t.reputationFieldMalware}
            value={item.malwareFamily}
          />
        )}
        {item.detail && (
          <MetaRow
        ...
        {item.raw && (
          <MetaRow
            label={t.reputationFieldReturnCode}
            value={<span className="font-mono">{item.raw}</span>}
          />
        )}
      </div>
    </div>
...
function pointsBySource(contributions: ReputationSummary["contributions"]) {
  ...
  for (const contribution of contributions) {
    ...
    totals.set(
      ...
      (totals.get(contribution.sourceId) ?? 0) + contribution.points,
    );
  }
  return totals;
}
...
function SourceRow({
  ...
  const description = definition
    ? t.reputationSourceDescriptions[sourceId]
    : undefined;
...
export function ReputationChecker({
  ...
  const { loading, error, result, run, cancel, querySync } =
    useToolLookup<ReputationSummary>({
              ...
              <div>
                ...
                {result.rawScore > result.score && (
                  <p className="mt-0.5 text-[11px] text-muted-foreground">
                    {formatTemplate(t.reputationScoreCapped, {
                      count: result.rawScore,
                    })}
                  </p>
                )}
              </div>
          ...
          <Card className="p-5">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <ContextCell
                ...
                secondary={
                  result.network?.isp || result.network?.org || undefined
                }
              />
              <ContextCell
                icon={Network}
                ...
                primary={
                  result.networkContext
                    ? baseT.connectionTypes[
                        result.networkContext.connectionType
                      ]
                    : "-"
                }
          ...
          <Card className="gap-0 overflow-hidden py-0">
            ...
            >
              <div className="border-b px-5 py-3">
                <TabsList className="flex h-auto w-full flex-wrap justify-start gap-1">
                  <TabsTrigger value="all">
                    {t.reputationFilterAll} (
                    {formatNumber(evidenceGroups.all.length, locale)})
                  </TabsTrigger>
                  <TabsTrigger value="threats">
                    {t.reputationSectionThreats} (
                    {formatNumber(evidenceGroups.threats.length, locale)})
                  </TabsTrigger>
                  <TabsTrigger value="mail">
                    {t.reputationSectionMail} (
                    {formatNumber(evidenceGroups.mail.length, locale)})
                  </TabsTrigger>
                  <TabsTrigger value="network">
                    {t.reputationSectionNetwork} (
                    {formatNumber(evidenceGroups.network.length, locale)})
                  </TabsTrigger>
                </TabsList>
              </div>
              <TabsContent value="all" className="flex flex-col">
                <EvidenceList
                  items={evidenceGroups.all}
                  empty={t.reputationNoEvidence}
                  t={t}
                  locale={locale}
                />
              </TabsContent>
              <TabsContent value="threats" className="flex flex-col">
                <EvidenceList
                  items={evidenceGroups.threats}
                  empty={t.reputationNoThreatEvidence}
                  t={t}
                  locale={locale}
                />
              </TabsContent>
              <TabsContent value="mail" className="flex flex-col">
                <EvidenceList
                  items={evidenceGroups.mail}
                  empty={t.reputationNoMailEvidence}
                  t={t}
                  locale={locale}
                />
              </TabsContent>
